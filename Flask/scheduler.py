...
def generate_schedule(db_session):
    ...
    # Constraint: A room cannot be used for more than one class at the same time on the same day.
    for day in ['MWF', 'TTh']:
    ...
    # Constraint: Professors cannot teach during their unavailable times
    for prof in professors:
        # Iterate over each restricted time slot for the professor.
        for restricted_ts in prof.time_restrictions:
            day = restricted_ts.meeting_days
            ts_id = restricted_ts.id
            # Find all scheduling variables for this professor at the restricted day and time slot.
            prof_vars = [
                var for key, var in schedule_vars.items()
                if key[2] == prof.id and key[3] == day and key[4] == ts_id
            ]
            # Ensure the professor is not scheduled during the restricted time.
