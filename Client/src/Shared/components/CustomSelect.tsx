...
function CustomSelect({
  ...
  const [selectedOption, setSelectedOption] = useState<CustomOption>(
  ...
  return (
    <Form.Group className='mb-3' controlId={id} ref={ref}>
      ...
      <div className='relative'>
        ...
        >
          <div className='flex items-center'>{selectedOption.render()}</div>
          <IoMdArrowDropdown />
        </button>
          ...
          <ul className='absolute pl-0 z-10 w-full mt-1 overflow-auto bg-white border border-gray-300 rounded-md shadow-lg max-h-60'>
            {options.map((option) => (
              <li
                ...
                tabIndex={0}
                onKeyDown={(event) =>
                  event.key === 'Enter' && handleSelect(option)
                }
              >
                {option.render()}
              </li>
